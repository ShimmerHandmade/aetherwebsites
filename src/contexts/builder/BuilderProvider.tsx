...
export const BuilderProvider: React.FC<BuilderProviderProps> = ({
  ...
  // Memoized findElementById function to prevent circular dependencies
  const findElementById = useMemo(() => {
    return (id: string): BuilderElement | null => {
  ...
  const duplicateElement = useCallback((id: string) => {
    try {
      setElements(prevElements => {
        ...
        // Find element directly in this callback to avoid circular dependency
        const searchElements = (elementsToSearch: BuilderElement[]): BuilderElement | null => {
          for (const element of elementsToSearch) {
            if (element?.id === id) return element;
            if (element?.children && Array.isArray(element.children)) {
        ...
        const element = searchElements(prevElements);
  ...
  const setCurrentBreakpointCallback = useCallback((breakpoint: BreakpointType) => {
    setCurrentBreakpoint(breakpoint);
  }, []);

  const setPreviewBreakpointCallback = useCallback((breakpoint: BreakpointType) => {
  ...
  const value: BuilderContextType = {
    ...
    updatePageSettings,
    saveChanges,
    setCurrentBreakpoint: setCurrentBreakpointCallback,
    setPreviewBreakpoint: setPreviewBreakpointCallback,
  };
